...
# instantiate emails as a dictionary of all emails
...
# define function to store each count, email in the dictionary as a tuple in a list
def make_list():
    # swap the keys to be values and values to be keys
    temp_dict = {value: key for key, value in emails.items()}
    # initialize list of tuples to be a list, populated with the tuples returned by the .items method of the emails
    # dictionary
    list_of_tuples = list(temp_dict.items())
    # sorting the list is in ascending order, meaning that it will start with the lowest number of commits. By
    # subsequently calling reverse() on the list, we can print the 0th element to get the highest number.
    list_of_tuples.sort()
